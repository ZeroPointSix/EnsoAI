...
export const zhTranslations: Record<string, string> = {
  ...
  'Open Agent or Terminal tabs to start sessions, then view them here.':
  ...
  'Type your prompt here (@ file, paste images)…': '在此输入内容（可 @ 文件、粘贴图片）…',
  'Type your prompt or command here…': '在此输入提示词或命令…',
  'Type your command here…': '在此输入命令…',
  ...
  'Continue prompt': '继续提示词',
  'Plain text sent when you press Continue (no toggles or supplement)':
    '在看板输入框按 Ctrl+Shift+Enter 发送的纯文本（不拼接开关与模板芯片内容）',
  '{{count}} templates created': '已配置 {{count}} 个模板',
  'Reset defaults': '恢复寸止默认',
  'Add template': '添加模板',
  'Quick template': '快捷模板',
  'Edit template': '编辑模板',
  '(empty — clears supplement)': '（空内容 — 点击后清空输入框）',
  'Condition label': '条件名称',
  'When ON (append on send)': '开启时追加（发送时生效）',
  'When OFF (append on send)': '关闭时追加（发送时生效）',
  'Default switch state': '默认开关状态',
  'No templates yet': '还没有任何模板',
  'Load 寸止 defaults': '加载寸止预设模板',
  ...
  'Attach image': '附加图片',
  'Agent running': '运行中',
  'Canvas agent idle': '空闲',
  'Canvas agent working': '执行中',
  'Canvas agent blocked': '需处理',
  'Canvas agent completed': '已完成',
  'New output': '有新输出',
